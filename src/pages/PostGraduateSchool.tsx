...
const PostGraduateSchool = () => {
  ...
  const suggestions = searchQuery.length > 0
    ? postGraduatePrograms.filter(program => 
  ...
  const renderLanguageFlags = (languages: string[]) => {
    return (
      <div className="flex gap-2">
        {languages.map(lang => (
          <Badge key={lang} className="bg-accent/10 text-accent border-accent/20 font-medium px-3 py-1">
    ...
    <main className="min-h-screen bg-background mesh-gradient">
      ...
      <script type="application/ld+json">
        {JSON.stringify({
          ...
          "@type": "ItemList",
          "name": "LUISS Post-Graduate Programs",
          "description": "PhD programs at LUISS University",
          "numberOfItems": postGraduatePrograms.length,
          "itemListElement": postGraduatePrograms.map((prog, idx) => ({
            "@type": "Course",
            "position": idx + 1,
            "name": prog.title,
            "description": `${prog.category} PhD program at LUISS`,
            "provider": {
      ...
      <section className="relative py-20 md:py-32 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-20 left-10 w-72 h-72 bg-accent/10 rounded-full blur-[120px] animate-pulse-slow"></div>
          <div className="absolute bottom-20 right-10 w-96 h-96 bg-primary/10 rounded-full blur-[140px] animate-pulse-slow" style={{ animationDelay: '1.5s' }}></div>
        </div>

        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="max-w-5xl mx-auto text-center space-y-8 animate-fade-up">
            <div className="inline-flex items-center gap-2 px-6 py-3 rounded-full glass shadow-[var(--shadow-md)] text-accent text-sm font-semibold mb-6">
              <Award className="h-5 w-5" />
              <span>Post-Graduate Programs</span>
            </div>
            <h1 className="font-nunito uppercase text-5xl md:text-7xl lg:text-8xl font-bold leading-tight">
              <span className="text-gradient-primary">PhD Programs</span>
              <br />
              <span className="text-foreground">for Researchers</span>
            </h1>
            <p className="text-xl md:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
              Advance your research career with world-class doctoral education. Choose from{" "}
              <span className="text-foreground font-semibold">{postGraduatePrograms.length} PhD programs</span>{" "}
            ...
            <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto pt-6">
              <div className="glass rounded-2xl p-6 shadow-[var(--shadow-md)]">
                <div className="text-3xl font-bold text-gradient-primary mb-2">{postGraduatePrograms.length}</div>
                <div className="text-sm text-muted-foreground">PhD Programs</div>
              </div>
              <div className="glass rounded-2xl p-6 shadow-[var(--shadow-md)]">
                <div className="text-3xl font-bold text-gradient-accent mb-2">4.9</div>
                <div className="text-sm text-muted-foreground">Research Rating</div>
              </div>
              <div className="glass rounded-2xl p-6 shadow-[var(--shadow-md)]">
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col lg:flex-row gap-6 items-stretch">
              ...
              <div className="relative flex-1">
                <Search className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-accent/60 z-10" />
                <Input
                  type="text"
                  placeholder="Search PhD programs..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onFocus={() => setShowSuggestions(true)}
                  onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}
                  className="pl-14 h-14 glass text-base rounded-2xl border-border/50 focus:border-accent/50 transition-all duration-300 shadow-[var(--shadow-md)]"
                />
                
                {/* Suggestions Dropdown */}
                {showSuggestions && suggestions.length > 0 && (
                  <div className="absolute top-full mt-2 left-0 right-0 bg-card glass rounded-2xl border border-border/50 shadow-[var(--shadow-lg)] overflow-hidden z-50 animate-fade-in">
                    {suggestions.map((program) => (
                      <Link
                        key={program.id}
                        to={`/postgraduate/program/${program.id}`}
                        className="flex items-center gap-4 px-5 py-4 hover:bg-accent/5 transition-colors duration-200 border-b border-border/30 last:border-b-0"
                      >
              ...
              <div className="flex gap-4 flex-wrap lg:flex-nowrap">
                <Select value={sortBy} onValueChange={setSortBy}>
                  <SelectTrigger className="min-w-[180px] h-14 rounded-2xl glass border-border/50 hover:border-accent/50 transition-all duration-300 shadow-[var(--shadow-md)]">
            ...
            {sortedPrograms.length === 0 && (
              <div className="text-center py-24 glass rounded-3xl">
                ...
                <Button
                  ...
                  className="bg-accent text-white hover:bg-accent/90 px-8 py-6 rounded-xl text-base font-semibold shadow-[var(--shadow-md)] hover:shadow-[var(--shadow-lg)] transition-all duration-300"
                >
                  Clear All Filters
                </Button>
              </div>
